...
public class AzaleaCommand extends Command {

    private final List<AzaleaCommand> children;

    public AzaleaCommand(String name, AzaleaCommand... children) {
        super(name);
        this.children = List.of(children);
    }

    public List<AzaleaCommand> getChildren() {
        return children;
    }
    ...
    public final boolean execute(@Nonnull CommandSender sender, @Nonnull String label, String[] args) {
        try {
            ...
            Map.Entry<AzaleaCommand, Arguments> pair = getClosestMatch(sender, getChildren(), arguments, 0, null);
    ...
    private Map.Entry<AzaleaCommand, Arguments> getClosestMatch(CommandSender sender, List<AzaleaCommand> parents, Arguments arguments, int depth, AzaleaCommand node) {
        if (arguments.size() == depth) {
            return new AbstractMap.SimpleEntry<>(node, arguments.subArguments(depth));
        }

        AzaleaCommand child = parents.stream()
    ...
    public static void register(JavaPlugin plugin, Class<? extends AzaleaCommand> command) {
        try {
